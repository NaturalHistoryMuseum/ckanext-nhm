...
def replace_resource_file(resource_id, path):
    ...
    if resource.get('datastore_active', False):
        click.secho('Resource has an active datastore - cannot replace the file', fg='red')
        raise click.Abort()
    ...
    success('The download file for resource {} has been replaced with {}', resource_id, path)
    click.secho('A copy of the original resource file has been made at {}'.format(backup_path),
                fg='yellow')
