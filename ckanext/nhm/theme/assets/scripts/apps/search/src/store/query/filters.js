...
let filters = {
        ...
        queryfy: (state, getters) => (id, ignoreTemp) => {
            ...
            if (!isGroup) {
                ...
                if (children.length > 0) {
                    let childData = children.map(c => getters.queryfy(c.key, ignoreTemp))
    ...
    actions: {
        ...
        deleteTemporaryFilters(context) {
            let deleteCount = 0;
            d3.entries(context.state.items).filter(f => f.value.display.temp)
              .forEach(f => {
                    ...
                    let dequeryfy = (items, parent) => {
                        ...
                        items.forEach((i) => {
                            ...
                            if (Array.isArray(item.value) && context.rootState.schema.groups.includes(item.key)) {
                                ...
                                d3.entries(dequeryfy(item.value, groupId))
                                  .forEach((f) => {
