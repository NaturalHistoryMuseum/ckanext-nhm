<template>
    <div class="flex-container flex-column flex-center">
        ...
        <div class="flex-container flex-center flex-column" v-if="comparisonType === 'named_area'">
            <span>
                <label for="geoCategory">Category</label>
                <select v-model="geoCategory" id="geoCategory">
                    <option v-for="(options, cat) in namedAreas" :key="cat.id">{{ cat }}
                    </option>
                </select>
            </span>
            <span>
        ...
        <div
            ...
            <div class="flex-container flex-wrap flex-wrap-spacing">
                <span class="fields" v-for="(polygon, index) in values.custom_area"
            ...
            <textarea v-model="pastedGeoJson" v-if="useGeoJson"
                placeholder="Paste a list of MultiPolygon coordinates, e.g. [[[[1, 1], [0, 0], [1, 0], [1, 1]]]]">
            </textarea>
            <button @click="parseGeoJson" class="btn btn-primary">
                Set
            </button>
        </div>
        ...
        methods:  {
            ...
            setLatLng(event) {
                ...
                if (this.comparisonType === 'custom_area') {
                    ...
                    this.$set(this.values.custom_area, this.values.custom_area.length - 1, [this.currentPolygon]);
                }
            },
            resetMap() {
