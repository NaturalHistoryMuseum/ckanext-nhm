<template>
    <Popup classes="share-popup"
           :icon="status.slug.loading ? 'fa-pulse fa-spinner' : 'fa-share-alt'" label="Share"
           popup-id="show-share" :parent-toggle="showPopup" v-on:toggle="toggle">
        <div v-if="slug !== null">
            <p>Share this search:</p>
            <Copyable :copy-text="'https://' + shareUrl" :display-text="shareUrl"
                      :edit-button="canEditSlug" @edit="showEditSlug = !showEditSlug">
                <span
                    :style="{minWidth: ((22 + slug.length) * 9.6) + 'px', width: '100%', wordBreak: 'break-all', opacity: showEditSlug ? 0.5 : 1}">{{
                        shareUrl
                                                                                                                                                   }}</span>
            </Copyable>
            <Editable :edit-text="slug" :edit-status="status.slugEdit" v-if="showEditSlug"
                      @save="saveSlugEdit" />
