...
def find_author_split(value, record_dict):
    '''
    Given a string and a record attempts to determine where in the string an author is defined. If an author is found
    then this will return the index the author part starts, otherwise returns None.
...
class AuthorParserStage(BaseParserStage):
    ...
    def _extract(self, body, record_dict):
        '''
        Searches for the full author string, then breaks it up into smaller pieces (sections in brackets, individual
...
class SimpleFieldParserStage(BaseParserStage):
    ...
    def _extract(self, body, record_dict):
        '''
        If the value is at the end of the string, there is no point in continuing; otherwise, it looks for the first
        capitalised word after that value.
