...
def extract_species(record):
    '''
    Extract the species value from the given record. This is achieved by extracting the species from
    the `scientificName` field of the record (if there is one) and the removal of any extra details
    (such as the author) from this value.
...
def find_author_split(value, record_dict):
    '''
    Given a string and a record attempts to determine where in the string an author is defined. If
    an author is found then this will return the index the author part starts, otherwise returns
    None.
...
class AuthorParserStage(BaseParserStage):
    ...
    def _extract(self, body, record_dict):
        '''
        Searches for the full author string, then breaks it up into smaller pieces (sections in
        brackets, individual names) if that's not found.
        ...
        author_strings = [full_author] + [p.strip() for p in set(
            re.findall(r'\(([\w\s]+)\)', full_author) + re.findall(r'([\w.\s]+)', full_author))]
...
class SimpleFieldParserStage(BaseParserStage):
    '''
    A generic stage for searching for a certain field within the tag body (for the purposes of
    finding authors).
    '''
    ...
    def _meets_criteria(self, body, record_dict):
        '''
        Checks that the record contains a value for this field and that the value is present in the
        tag body.
        '''
    ...
    def _extract(self, body, record_dict):
        '''
        If the value is at the end of the string, there is no point in continuing; otherwise, it
        looks for the first capitalised word after that value.
        ...
        if re.search(f'{re.escape(field_value)}$', body):
            return len(body)
        split_by_value = re.split(f'{re.escape(field_value)}', body,
                                  1)
