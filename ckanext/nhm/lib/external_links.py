...
class Site(object):
    ...
    def rank_links(self, record):
        ...
        if ranks:
            return OrderedDict.fromkeys([(rank, self.get_link(rank)) for rank in ranks.values()])
        else:
            return []
...
def _p10k_api(gbif_record):
    ...
    r = requests.get('https://www.phenome10k.org/api/v1/scan/search',
...
def _get_gbif_record(record):
    ...
    r = requests.get('https://api.gbif.org/v1/occurrence/search',
                     params={'occurrenceID': record.get('occurrenceID'),
...
def get_gbif_links(record):
    ...
    if 'acceptedTaxonKey' in gbif_record:
        gbif_links.append((gbif_record.get('scientificName'),
